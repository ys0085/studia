...
procedure  Peterson_Template is
...
Flag : array (0 .. 1) of Boolean := (others => False);
Turn : Integer := 0;
...
task body Process_Task_Type is
   ...
   -- These must be declared as shared variables outside the task body,
   -- so we declare them as 'protected' objects below the task type.
   -- But for this template, we use them as package-level variables.
   -- So, declare them here as 'shared' with pragma Atomic/Volatile in the main spec.

   -- We'll use the following declarations at the package level (outside the task body):
   -- Flag : array (0 .. 1) of Boolean := (others => False);
   -- Turn : Integer := 0;
   ...
   -- Helper to get the other process's index
...
begin
   ...
   for Step in 0 .. Nr_of_Steps/4 - 1  loop  -- TEST !!!
      ...
      -- Peterson's ENTRY_PROTOCOL
      Flag(Process.Id) := True;
      Turn := Other_Id;
      while Flag(Other_Id) and then Turn = Other_Id loop
         null; -- busy wait
      end loop;
